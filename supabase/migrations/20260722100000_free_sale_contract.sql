-- Migration: add free sale contracts.

alter table public.finance_contracts
  add column if not exists free_sale_data jsonb;

alter table public.finance_contracts
  drop constraint if exists finance_contracts_contract_type_check;

alter table public.finance_contracts
  add constraint finance_contracts_contract_type_check
  check (
    contract_type is null
    or contract_type in ('عقد بيع', 'عقد تقسيط', 'عقد بيع حر')
  );
