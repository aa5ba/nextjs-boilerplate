...
export async function GET() {
  try {
    ...
    if (error) {
      console.error(
        "Admin support Ehtisab providers query failed:",
        error
      );
    ...
    console.error(
      "Admin support Ehtisab providers GET error:",
      error
    );
  ...
  try {
    ...
    const { data, error } =
      await supabaseAdmin.rpc(
        "create_ehtisab_finance_provider_atomic",
        {
          p_provider_name:
            providerName,
          p_display_order:
    ...
    if (error) {
      ...
      console.error(
        "create_ehtisab_finance_provider_atomic failed:",
        {
          code:
            error.code,
          message:
    ...
    const result =
      ...
      typeof rawResult === "object"
        ? (rawResult as ProviderResult)
        : null;

    if (!result?.provider_id) {
      console.error(
        "create_ehtisab_finance_provider_atomic returned invalid data:",
        data
      );
    ...
    console.error(
      "Admin support Ehtisab providers POST error:",
      error
    );

    return createErrorResponse(
      "حدث خطأ أثناء حفظ جهة التمويل",
      500
    );
  }
}
