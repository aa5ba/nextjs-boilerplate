...
export async function GET(
  ...
  try {
    ...
    if (error) {
      console.error(
        "Admin support Ehtisab margin rules query failed:",
        error
      );

      return createErrorResponse(
        "تعذر تحميل قواعد هامش الربح",
        500
      );
    }
    ...
    console.error(
      "Admin support Ehtisab margin rules GET error:",
      error
    );

    return createErrorResponse(
      "حدث خطأ أثناء تحميل قواعد هامش الربح",
      500
    );
  }
}
  ...
  try {
    ...
    const { data, error } =
      await supabaseAdmin.rpc(
        "create_ehtisab_margin_rule_atomic",
        {
          p_provider_id:
            rule.providerId,
          p_finance_type:
    ...
    if (error) {
      ...
      console.error(
        "create_ehtisab_margin_rule_atomic failed:",
        {
          code:
            error.code,
          message:
    ...
    console.error(
      "Admin support Ehtisab margin rules POST error:",
      error
    );

    return createErrorResponse(
      "حدث خطأ أثناء حفظ قاعدة الهامش",
      500
    );
  }
}
