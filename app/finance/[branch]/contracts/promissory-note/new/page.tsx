...
export default function NewPromissoryNotePage() {
  ...
  async function createNote() {
    ...
    if (!debtorName || !amount || !dueDate || !city) {
      alert("أكمل اسم المدين ومبلغ السند وتاريخ الاستحقاق والمدينة");
      return;
    }
    ...
    try {
      ...
      const { data: noteSequenceData, error: sequenceError } =
      ...
      router.push(
        `/finance/${branch}/contracts/promissory-note/print/${data.id}`
      );
    } catch (error: any) {
      alert(error.message || "تعذر إنشاء السند");
    } finally {
      setSaving(false);
    }
  }
    ...
    <main dir="rtl" style={getPageStyle(isMobile)}>
      <div style={getContainerStyle(isCompact)}>
        <header style={getHeroStyle(isMobile)}>
          ...
          <div style={getHeroContentStyle(screen)}>
            ...
            <div style={getHeroTitleBoxStyle(screen)}>
              <h1 style={getTitleStyle(screen)}>إنشاء سند جديد</h1>
            </div>
        ...
        <section style={card}>
          ...
          <button style={primaryButton} onClick={createNote} disabled={saving}>
            {saving ? "جاري الإنشاء..." : "إنشاء السند"}
          </button>
        </section>
