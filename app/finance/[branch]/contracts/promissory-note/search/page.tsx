  ...
  async function searchNotes() {
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .or(
          `note_number.eq.${normalizedSearch},debtor_name.ilike.%${cleanedSearch}%,debtor_national_id.ilike.%${normalizedSearch}%,debtor_phone.ilike.%${normalizedSearch}%`
        )
        .order("created_at", { ascending: false });

      if (error) {
        alert(error.message || "تعذر البحث عن السند");
        return;
      }
  ...
  return (
    <main dir="rtl" style={getPageStyle(isMobile)}>
      <div style={getContainerStyle(isCompact)}>
        <header style={getHeroStyle(isMobile)}>
          ...
          <div style={getHeroContentStyle(screen)}>
            ...
            <div style={getHeroTitleBoxStyle(screen)}>
              <h1 style={getTitleStyle(screen)}>البحث عن سند</h1>
            </div>
        ...
        <section style={card}>
          <div style={isCompact ? searchRowCompact : searchRow}>
            <input
              style={input}
            ...
            <button style={searchButton} onClick={searchNotes} disabled={searching}>
        ...
        <div style={backWrapper}>
          <button
            style={backButton}
            onClick={() => router.push(`/finance/${branch}/contracts`)}
          >
            الرجوع للعقود
          </button>
        </div>
...
const searchRowCompact: CSSProperties = {
  display: "grid",
  gridTemplateColumns: "1fr",
  gap: 12,
};
