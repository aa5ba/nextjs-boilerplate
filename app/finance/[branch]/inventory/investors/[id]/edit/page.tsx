...
export default function EditInvestorPage() {
  ...
  async function initializePage() {
    ...
    if (!hasPermissionFromAccess(userAccess.roles, userAccess.permissions, "edit_investor")) {
      alert("لا تملك صلاحية تعديل المستثمرين");
      router.replace(`/finance/${branch}/inventory/investors/${investorId}`);
      return;
    }
  ...
  async function loadInvestor() {
    ...
    if (error || !data) {
      alert(error?.message || "لم يتم العثور على المستثمر");
      setLoading(false);
      return;
    }
        ...
        <header style={getHeroStyle(isMobile)}>
          ...
          <div style={getHeroContentStyle(screen)}>
            ...
            <div style={getHeroTitleBoxStyle(screen)}>
              <h1 style={getTitleStyle(screen)}>تعديل المستثمر</h1>
            </div>
