...
export default function NewProductPage() {
  ...
  async function saveProduct() {
    ...
    try {
      ...
      const { data: duplicatedProduct, error: duplicateError } = await supabase
      ...
      if (duplicateError) {
        alert(duplicateError.message || "تعذر التحقق من تكرار المنتج");
        return;
      }

      if (duplicatedProduct) {
        alert("هذا المنتج موجود مسبقاً في هذا الفرع");
        return;
      }
      ...
      const { error } = await supabase.from("finance_products").insert([
        {
          ...
          product_category: cleanCategory || null,
      ...
      router.push(`/finance/${branch}/inventory/products`);
    } finally {
  ...
  if (!authChecked || !permissionLoaded) {
    return null;
  }
      ...
      <main dir="rtl" style={getPageStyle(isMobile)}>
        <div style={getContainerStyle(isCompact)}>
          <header style={getHeroStyle(isMobile)}>
            ...
            <div style={getHeroContentStyle(screen)}>
              ...
              <div style={getHeroTitleBoxStyle(screen)}>
                <h1 style={getTitleStyle(screen)}>إضافة منتج</h1>
              </div>
          ...
          <section style={deniedCard}>
            ...
            <p style={{ marginBottom: 0 }}>ليس لديك صلاحية إضافة المنتجات.</p>
          </section>
