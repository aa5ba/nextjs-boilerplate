...
type Payment = {
  ...
  finance_contracts?: ContractRelation | ContractRelation[] | null;
};
...
export default function FinancePaymentsPage() {
  ...
  const verifyEmployeeInBackground = useCallback(
    async (
  ...
  function getCancelErrorMessage(message: string) {
    ...
    if (
      message.includes("PAYMENT_ALREADY_CANCELLED")
    ) {
    ...
    if (
      message.includes("INVALID_EMPLOYEE_SESSION")
    ) {
      return "انتهت جلسة الموظف، سجل الدخول مرة أخرى";
    }

    return message || "تعذر إلغاء عملية السداد";
  }

  async function cancelPayment(payment: Payment) {
    if (!branchId || cancellingPaymentId) {
      if (!branchId) {
    ...
    const contract =
      getSingleContract(payment.finance_contracts);
    ...
    try {
      ...
      if (error) {
        throw new Error(
          getCancelErrorMessage(error.message)
        );
      }
      ...
      const result =
        rawResult as CancelPaymentResult | null;
      ...
      alert(getCancelErrorMessage(message));
    } finally {
      ...
      >
        ...
        <section style={card}>
          ...
          >
            ...
            {loading ? (
              <div style={emptyBox}>
                جاري تحميل عمليات السداد...
              </div>
            ) : payments.length === 0 ? (
              <div style={emptyBox}>
                لا توجد عمليات سداد حتى الآن
              </div>
            ) : (
