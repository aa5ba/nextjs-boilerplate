...
export default function FinanceCustomerGroupsPage() {
  ...
  async function addGroup() {
    ...
    const duplicateGroup = groups.find(
      (group) => String(group.name || "").trim() === cleanName
    );

    if (duplicateGroup) {
      alert("هذه المجموعة موجودة مسبقًا");
      return;
    }
    ...
    try {
      ...
      alert(error.message || "تعذر إنشاء المجموعة");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function editGroup(group: any) {
    ...
    const duplicateGroup = groups.find(
      (item) =>
        item.id !== group.id && String(item.name || "").trim() === cleanName
    );

    if (duplicateGroup) {
      alert("توجد مجموعة أخرى بنفس الاسم");
      return;
    }
    ...
    try {
      ...
      alert(error.message || "تعذر تعديل المجموعة");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function deleteGroup(group: any) {
    ...
    try {
      ...
      alert(
        error.message ||
          "تعذر حذف المجموعة. قد تكون مرتبطة بعملاء داخل النظام."
      );
    } finally {
      setSaving(false);
    }
    ...
    <main dir="rtl" style={getPageStyle(isMobile)}>
      <div style={getContainerStyle(isCompact)}>
        <header style={getHeroStyle(isMobile)}>
          ...
          <div style={getHeroContentStyle(screen)}>
            ...
            <div style={getHeroTitleBoxStyle(screen)}>
              <h1 style={getTitleStyle(screen)}>إنشاء / تعديل مجموعة عملاء</h1>
            </div>
        ...
        <section style={card}>
          ...
          <button style={primaryButton} onClick={addGroup} disabled={saving}>
            {saving ? "جاري المعالجة..." : "إنشاء مجموعة"}
          </button>
        </section>

        <section style={card}>
          {loading ? (
            <div style={emptyBox}>جاري تحميل المجموعات...</div>
          ) : groups.length === 0 ? (
