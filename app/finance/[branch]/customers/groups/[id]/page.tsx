...
export default function FinanceCustomerGroupDetailsPage() {
  ...
  async function loadData() {
    ...
    if (groupError) {
      alert(groupError.message || "تعذر تحميل بيانات المجموعة");
      setGroup(null);
      setCustomers([]);
      setLoading(false);
      return;
    ...
    if (customersError) {
      alert(customersError.message || "تعذر تحميل عملاء المجموعة");
      setGroup(groupData);
      setCustomers([]);
      setLoading(false);
      return;
    ...
    return (
      <main dir="rtl" style={getPageStyle(isMobile)}>
        <div style={getContainerStyle(isCompact)}>
          <header style={getHeroStyle(isMobile)}>
            ...
            <div style={getHeroContentStyle(screen)}>
              ...
              <div style={getHeroTitleBoxStyle(screen)}>
                <h1 style={getTitleStyle(screen)}>مجموعة العملاء</h1>
              </div>
          ...
          <div style={loadingBox}>جاري تحميل المجموعة...</div>
        </div>
      </main>
    );
  }
          ...
          {customers.length === 0 ? (
            ...
            customers.map((customer) => (
              <div
                ...
                style={tableRow}
                onClick={() =>
                  router.push(`/finance/${branch}/customers/${customer.id}`)
                }
              >
