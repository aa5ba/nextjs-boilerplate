  ...
  try {
    ...
    const session =
      await requireFinanceBranchSession({
        ...
        requiredPermission:
          "initialize_investor_goods_cost",
      });
    ...
    const { data, error } =
      await supabaseAdmin.rpc(
        "initialize_investor_goods_cost_secure_atomic",
        {
          p_branch_id:
            session.branchId,
          p_actor_user_id:
    ...
    return createResponse({
      ok: true,
      message:
        "تم تحديد تكلفة السلع الافتتاحية",
    ...
    console.error(
      "Investor goods initialize cost error:",
      error
    );

    return createErrorResponse(
      "تعذر تحديد تكلفة السلع",
      500,
      "GOODS_COST_INIT_FAILED"
    );
  }
}
